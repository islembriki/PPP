...
# ⚡ SET THIS TO True TO SKIP RECOMPUTING t-SNE (uses saved files)
REUSE_TSNE = True  # <--- THIS IS WHAT SAVES YOU 2 HOURS
...
# ============================================
# STEP 6: APPLY t-SNE (or load saved result)  ⚡ TIME SAVER
# ============================================
...
if REUSE_TSNE and os.path.exists(tsne_path):
    # ⚡ LOAD SAVED — skips the 2 hour computation
    ...
    # COMPUTE from scratch (only runs if REUSE_TSNE=False or no saved file)
    print("\n[STEP 6] Applying t-SNE (this will take a while)...")
    tsne = TSNE(
        n_components=2,
        random_state=42,
        perplexity=30,
    ...
    # ⚡ SAVE so you never have to recompute
...
mode_names  = {0: "No Drone", 1: "Connected", 2: "Hovering", 3: "Flying NoVid", 4: "Flying WithVid"}  # FIXED: added 0
...
mode_colors = {
    0: '#808080',  # Gray   - No Drone (Background)   FIXED: added 0
...
for label_id in range(4):
    for mode_id in range(0, 5):   # FIXED: start from 0
...
for mode_id in sorted(np.unique(mode_clean)):
    ...
    plt.scatter(X_tsne[indices, 0], X_tsne[indices, 1],
                c=[mode_colors[mode_id]], label=mode_names[mode_id],   # FIXED: mode_id=0 now exists
                alpha=0.7, s=50, edgecolors='k', linewidth=0.3)
